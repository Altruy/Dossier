...
const Calenda = ({ navigation}) => {
    ...
    useEffect(() => {
        ...
        showFun({"dateString": ret , "day": date , "month": month , "year" : dat.getFullYear() , "timestamp" : dat.getTime()})
    }, [])

    const get = () => getEvents(collabId,year,mon).then((val)=>setdata(val.map((v)=>{
